...
template<typename T>
struct ArrayOfPolarCoordinates
{
    T radius;
    T phi;
};
...
void SoA_RestScalar(benchmark::State &state)
{
 ...
 float_v phi_v;
        ...
        while(state.KeepRunning())
        {
            ...
            {
                //!Scalar Calculation
                std::tie(outputValues.radius[n], outputValues.phi[n]) = calcularePolarCoordinate(inputValues.x[n], inputValues.y[n]);
            }
        }
...
{
 //!The label for the plotter
 const std::string label(getLabelString("SoA_LoadStore_RestScalar/", state.range_x(), 1));
 //!The size of the values to process
 const size_t inputSize = state.range_x();
 //!The size of the values without a full vc-vector
 const size_t missingSize = (inputSize%float_v::size());
